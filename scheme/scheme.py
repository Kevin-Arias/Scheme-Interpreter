...
def make_call_frame(procedure, args, env):
    ...
    elif type(procedure) is MuProcedure:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN Question 10
        if same_length(formals, vals):
            definer(formals, vals, child)
        else:
            raise SchemeError
        # END Question 10
        return child
...
def same_length(list1, list2):
    if list1 is nil and list2 is nil:
        return True
    elif list1 is nil or list2 is nil:
        return False
    else:
        return same_length(list1.second, list2.second)

def definer(list1, list2, env):
    if list1 is not nil:
        env.define(list1.first, list2.first)
        if list1.second is not nil:
            definer(list1.second, list2.second, env)
...
def check_formals(formals):
    ...
    elif same_element(formals):
        raise SchemeError('symbol is repeated')
    elif formals.second != nil:
        check_formals(formals.second)



    # END Question 11B
def pair_to_list(paired_lst):
    true_list = []
    if paired_lst.second is nil:
        true_list.append(paired_lst.first)
        return true_list
    else:
        true_list.append(paired_lst.first)
        for element in pair_to_list(paired_lst.second):
            true_list.append(element)
        return true_list

def same_element(lst):
    true_false_list = []
    proper_list = pair_to_list(lst)
    index = 1
    for true_element in proper_list:
        if true_element in proper_list[index:]:
